...
let to_string { snake; snake2; game_state; apple; board; score } =
  Core.sprintf
...
let create ~height ~width ~initial_snake_length =
  ...
  let snake2 = Snake.create ~length:initial_snake_length in
  let apple = Apple.create ~board ~snake in
  match apple with
  | None -> failwith "unable to create initial apple"
  | Some apple ->
    let t = { snake; snake2; apple; game_state = In_progress; board; score = 0 } in
...
let handle_key t key =
  ...
  | None -> ()
  | Some ex -> Snake.set_direction t.snake ex
;;
...
let check_for_collisions t =
  ...
  if not collision then t.game_state <- Game_over "Out of bounds!" else ()
;;
...
let maybe_consume_apple t =
  ...
  then (
    ...
    (*Put another apple*)
    t.score <- t.score + Apple.score_by_color t.apple;
    match Apple.create ~board:t.board ~snake:t.snake with
    | None -> t.game_state <- Game_state.Win_snake2 "WIN 1"
    | Some apple -> t.apple <- apple
    (*the second snake eat the apple*))
  else if Position.equal (Snake.head t.snake2) (Apple.location t.apple)
  then Snake.grow_over_next_steps t.snake2 (Apple.amount_to_grow t.apple);
  (*Put another apple*)
  t.score <- t.score + Apple.score_by_color t.apple;
  match Apple.create ~board:t.board ~snake:t.snake with
  | None -> t.game_state <- Game_state.Win_snake1 "WIN 2"
  | Some apple -> t.apple <- apple
;;
...
let step t =
  if Snake.step t.snake && Snake.step t.snake2
  then (
