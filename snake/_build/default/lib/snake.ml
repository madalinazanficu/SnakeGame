...
let create ~length =
  ...
  ; { direction = Left
    ; extensions_remaining = 0
    ; head = { Position.row = length - 1; col = 0 }
    ; tail = List.init (length - 1) ~f:(fun col -> { Position.row = 0; col })
    }
  ]
;;
...
let move_forward head tail direction =
  let rest = List.tl_exn tail in
  let last_head = head in
  Direction.next_position direction head, rest @ [ last_head ]
;;
...
let collides_with_self t =
  let not_collide element = if not (Position.equal t.head element) then true else false in
  not (List.for_all ~f:not_collide t.tail)
;;
